...
async function detectDuplicates(matches, page) {
    ...
    console.log((await Promise.all(planned.map(p => p.getProperty("textContent")))).map(t => t ? t.jsonValue() : 'Not FOUND'), recordings);
...
async function checkAllTimeings(panned, page) {
    const recordings = [];
    for (const recording of panned) {
        recordings.push(await checkTimeing(recording, page));
    }
    return recordings;
}
