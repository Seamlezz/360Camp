...
async function createEvent(page, data) {
    ...
    await page.type('#title', `${data.team} - ${data.guestClub}`);
    ...
    const [button] = await page.$x("//button[contains(., 'Plan recording')]");
    if (button) {
        await Promise.all([button.click(), page.waitForSelector('a[alt="Return"]')]);
    }
}
async function addTeam(page, data) {
    const { team, guestClub } = data;
    await page.click(`div[alt="Edit ${team} - ${guestClub}"]`);
